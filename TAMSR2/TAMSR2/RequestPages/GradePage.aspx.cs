...
namespace TAMSR2.RequestPages
{
    public partial class GradePage : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            path = url.Substring(0, url.IndexOf(@"RequestPages/GradePage.aspx"));
        ...
        public string GetByID()
        {
            ...
            try
            {
                ...
                dv = objDAL.Connection.ExecuteProcedure("TAMSR2_GradeOps", ht);

                if (dv != null)
                {
                    obj.ID = Numerics.GetDecimal(dv[0]["grade_id"]);
                    obj.NoOfAl = Numerics.GetInt(dv[0]["number_of_Al"] + "");
                    obj.NoOfCl = Numerics.GetInt(dv[0]["number_of_Cl"] + "");
                    obj.NoOfSl = Numerics.GetInt(dv[0]["number_of_Sl"] + "");
                    obj.OvertimeEligible = dv[0]["overtime_eligible_flag"] + "";
                    obj.SeniorFlag = dv[0]["senior_flag"] + "";
                    obj.Code = dv[0]["code"] + "";
        ...
        public string GetIdNameListString()
        {
            ...
            try
            {
                ...
                string gradeStr = (String)GetGlobalResourceObject("Resource", "grade");

                sb.Append("<option value=''>" + chooseStr + " " + gradeStr + "</option>");

                foreach (DataRowView row in dv)
                {
                    Grade obj = new Grade();
                    obj.ID = Numerics.GetDecimal(row["grade_id"] + "");
        ...
        public string DeleteByID()
        {
            ...
            try
            {
                ...
                int temp = objDAL.Connection.ExecuteNonQueryProcedure("TAMSR2_GradeOps", ht);
                if (temp > 0)
                    return "1";
                else
                    return "0";
            ...
            {
                new ExceptionLog().WriteLog("RequestPages/GradePage.aspx.DeleteByID()", 0, ex);
            }
            finally
            {
                objDAL.CloseConnection();
        ...
        public string GetAll()
        {
            ...
            try
            {
                ...
                string[] cols = { "gd.grade_id", "gd.code", "gd.description" + languagePostfix, "gd.overtime_eligible_flag" , "gd.senior_flag", "gd.number_of_Cl", "gd.number_of_Sl", "gd.number_of_Al", "gd.last_updated_date", "gd.grade_id" };
                ...
                ht.Add("endRow", end);
                DataView objDataView = objDAL.Connection.ExecuteProcedure("TAMSR2_GradeOps", ht);
                ...
                ht.Add("searchInput", (Request["sSearch"] + "").Trim());
                ht.Add("orderBy", orderByClause);
                json.iTotalRecords = Numerics.GetInt(objDAL.Connection.GetValueProcedure("TAMSR2_GradeOps", ht));
                json.iTotalDisplayRecords = json.iTotalRecords;

                foreach (DataRowView obj in objDataView)
                {
                    List<string> tempList = new List<string>();
                    tempList.Add("<input type='checkbox' class='checkboxes' value='" + Numerics.GetDecimal(obj["grade_id"]) + "' />");
                    ...
                    if (Numerics.GetBool(obj["overtime_eligible_flag"] + "") == true)
                        tempList.Add("<i class='icon-ok'></i>");
                    else
                        tempList.Add("<i class='icon-remove'></i>");
                    if (Numerics.GetBool(obj["senior_flag"] + "") == true)
                        tempList.Add("<i class='icon-ok'></i>");
                    else
                        tempList.Add("<i class='icon-remove'></i>");
                    tempList.Add(obj["number_of_Cl"] + "");
                    tempList.Add(obj["number_of_Sl"] + "");
                    tempList.Add(obj["number_of_Al"] + "");
                    ...
                    {
                        tempAction = "<img src='" + path + "images/edit.png' title='Edit' alt='Edit' onclick='return Edit(" + Numerics.GetDecimal(obj["grade_id"]) + ");' /> &nbsp; ";
                    }
                    if (Numerics.GetBool(Session["CanDelete"]) == true || true)
                    {
                        //tempAction += "<img src='" + path + "images/delete.png' alt='Delete' title='Delete' onclick='Delete(" + Numerics.GetDecimal(obj["grade_id"]) + ")' /> ";
                    }
            ...
            {
                new ExceptionLog().WriteLog("RequestPages/GradePage.aspx.GetAll()", 0, ex);
            }
            finally
            {
                objDAL.CloseConnection();
        ...
        {
            string chk = "0";
            string overTimeEligible = "N";
            string seniorFlag = "N";
            if ((Request["overtimeEligible"] + "").ToLower() == "true")
                overTimeEligible = "Y";
            if ((Request["seniorFlag"] + "").ToLower() == "true")
                seniorFlag = "Y";
            ...
            ht.Add("noOfAl", (Request["noOfAl"] + "").Trim());
            ht.Add("noOfCl", (Request["noOfCl"] + "").Trim());
            ht.Add("noOfSl", (Request["noOfSl"] + "").Trim());
            ht.Add("overtimeEligibleFlag", overTimeEligible);
            ht.Add("seniorFlag", seniorFlag);
            ...
            try
            {
                ...
                string result = objDAL.Connection.GetValueProcedure("TAMSR2_GradeOps", ht);
            ...
            {
                new ExceptionLog().WriteLog("RequestPages/GradePage.aspx.Save()", 0, ex);
            }
            finally
            {
                objDAL.CloseConnection();
