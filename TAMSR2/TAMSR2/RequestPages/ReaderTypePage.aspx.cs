...
{
    public partial class ReaderTypePage : System.Web.UI.Page
    {
        string url = "";
        string path = "";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            path = url.Substring(0, url.IndexOf(@"RequestPages/ReaderTypePage.aspx"));
        ...
        public string GetByID()
        {
            ...
            try
            {
                ...
                dv = objDAL.Connection.ExecuteProcedure("TAMSR2_ReaderTypeOps", ht);

                if (dv != null)
                {
                    obj.ID = Numerics.GetDecimal(dv[0]["reader_type_id"]);
                    obj.NameEn = dv[0]["type_name_Eng"] + "";
                    obj.NameAr = dv[0]["type_name_Arb"] + "";
                    obj.DriverNameEn = dv[0]["driver_name_Eng"] + "";
                    obj.DriverNameAr = dv[0]["driver_name_Arb"] + "";
                    obj.LoadDriverFlag = Numerics.GetInt(dv[0]["load_driver_flag"]);
        ...
        public string GetIdNameListString()
        {
            ...
            try
            {
                ...
                string readerTypeStr = (String)GetGlobalResourceObject("Resource", "readerType");

                sb.Append("<option value=''>" + chooseStr + " " + readerTypeStr + "</option>");

                foreach (DataRowView row in dv)
                {
                    ReaderType obj = new ReaderType();
                    obj.ID = Numerics.GetDecimal(row["reader_type_id"] + "");
                    obj.DescriptionEn = row["type_name_Eng"] + "";
                    obj.DescriptionAr = row["type_name_arb"] + "";
                    if (cookie.Values["language"] == "ar")
        ...
        public string DeleteByID()
        {
            ...
            try
            {
                ...
                int temp = objDAL.Connection.ExecuteNonQueryProcedure("TAMSR2_ReaderTypeOps", ht);
                if (temp > 0)
                    return "1";
                else
                    return "0";
            ...
            {
                new ExceptionLog().WriteLog("RequestPages/ReaderTypePage.aspx.DeleteByID()", 0, ex);
            }
            finally
            {
                objDAL.CloseConnection();
        ...
        public string GetAll()
        {
            ...
            try
            {
                ...
                string[] cols = { "reader_type_id", "type_name" + languagePostfix, "driver_name" + languagePostfix, "load_driver_flag", "last_updated_date", "reader_type_id" };
                ...
                ht.Add("endRow", end);
                DataView objDataView = objDAL.Connection.ExecuteProcedure("TAMSR2_ReaderTypeOps", ht);
                ...
                ht.Add("searchInput", (Request["sSearch"] + "").Trim());
                ht.Add("orderBy", orderByClause);
                json.iTotalRecords = Numerics.GetInt(objDAL.Connection.GetValueProcedure("TAMSR2_ReaderTypeOps", ht));
                json.iTotalDisplayRecords = json.iTotalRecords;

                foreach (DataRowView obj in objDataView)
                {
                    List<string> tempList = new List<string>();
                    tempList.Add("<input type='checkbox' class='checkboxes' value='" + Numerics.GetDecimal(obj["reader_type_id"]) + "' />");
                    tempList.Add((obj["type_name" + languagePostfix] + "").Length > 100 ? (obj["type_name" + languagePostfix] + "").Substring(0, 100) + "..." : (obj["type_name" + languagePostfix] + ""));
                    tempList.Add((obj["driver_name" + languagePostfix] + "").Length > 100 ? (obj["driver_name" + languagePostfix] + "").Substring(0, 100) + "..." : (obj["driver_name" + languagePostfix] + ""));
                    tempList.Add(Numerics.GetBool(obj["load_driver_flag"] + "") + "");
                    ...
                    {
                        tempAction = "<img src='" + path + "images/edit.png' title='Edit' alt='Edit' onclick='return Edit(" + Numerics.GetDecimal(obj["reader_type_id"]) + ");' /> &nbsp; ";
                    }
                    if (Numerics.GetBool(Session["CanDelete"]) == true || true)
                    {
                        //tempAction += "<img src='" + path + "images/delete.png' alt='Delete' title='Delete' onclick='Delete(" + Numerics.GetDecimal(obj["reader_type_id"]) + ")' /> ";
                    }
            ...
            {
                new ExceptionLog().WriteLog("RequestPages/ReaderTypePage.aspx.GerAll()", 0, ex);
            }
            finally
            {
                objDAL.CloseConnection();
        ...
        {
            string chk = "0";
            int loadDriverFlag = 0;
            if ((Request["loadDriverFlag"] + "").ToLower() == "true")
                loadDriverFlag = 1;
            Hashtable ht = new Hashtable();
            ht.Add("action", "save");
            ht.Add("sessionID", Session["ID"] + "");
            ht.Add("id", (Request["id"] + "").Trim());
            ht.Add("typeNameEng", (Request["nameEn"] + "").Trim());
            ht.Add("typeNameArb", (Request["nameAr"] + "").Trim());
            ht.Add("driverNameEng", (Request["driverEn"] + "").Trim());
            ht.Add("driverNameArb", (Request["driverAr"] + "").Trim());
            ht.Add("loadDriverFlag", loadDriverFlag);
            ...
            try
            {
                ...
                string result = objDAL.Connection.GetValueProcedure("TAMSR2_ReaderTypeOps", ht);
            ...
            {
                new ExceptionLog().WriteLog("RequestPages/ReaderTypePage.aspx.Save()", 0, ex);
            }
            finally
            {
                objDAL.CloseConnection();
