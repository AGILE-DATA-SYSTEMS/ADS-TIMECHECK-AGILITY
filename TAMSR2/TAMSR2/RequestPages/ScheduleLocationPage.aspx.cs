...
{
    public partial class ScheduleLocationPage : System.Web.UI.Page
    {
        string url = "";
        string path = "";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            path = url.Substring(0, url.IndexOf(@"RequestPages/ScheduleLocationPage.aspx"));
        ...
        public string GetByID()
        {
            ...
            try
            {
                ...
                dv = objDAL.Connection.ExecuteProcedure("TAMSR2_ScheduleLocationOps", ht);

                if (dv != null)
                {
                    obj.ID = Numerics.GetDecimal(dv[0]["schedule_location_id"]);
        ...
        public string GetIdNameListString()
        {
            ...
            try
            {
                ...
                string orgTypeStr = (String)GetGlobalResourceObject("Resource", "scheduleLocation");
                ...
                {
                    OrgType obj = new OrgType();
                    obj.ID = Numerics.GetDecimal(row["schedule_location_id"] + "");
        ...
        public string DeleteByID()
        {
            ...
            try
            {
                ...
                int temp = objDAL.Connection.ExecuteNonQueryProcedure("TAMSR2_ScheduleLocationOps", ht);
                if (temp > 0)
                    return "1";
                else
                    return "0";
            ...
            {
                new ExceptionLog().WriteLog("RequestPages/ScheduleLocationPage.aspx.DeleteByID()", 0, ex);
            }
            finally
            {
                objDAL.CloseConnection();
        ...
        public string GetAll()
        {
            ...
            try
            {
                ...
                string[] cols = { "schedule_location_id", "description" + languagePostfix, "last_updated_date", "schedule_location_id" };
                ...
                ht.Add("endRow", end);
                DataView objDataView = objDAL.Connection.ExecuteProcedure("TAMSR2_ScheduleLocationOps", ht);
                ...
                ht.Add("searchInput", (Request["sSearch"] + "").Trim());
                ht.Add("orderBy", orderByClause);
                json.iTotalRecords = Numerics.GetInt(objDAL.Connection.GetValueProcedure("TAMSR2_ScheduleLocationOps", ht));
                json.iTotalDisplayRecords = json.iTotalRecords;

                foreach (DataRowView obj in objDataView)
                {
                    List<string> tempList = new List<string>();
                    tempList.Add("<input type='checkbox' class='checkboxes' value='" + Numerics.GetDecimal(obj["schedule_location_id"]) + "' />");
                    ...
                    {
                        if (new TAMSPrivilege().IsPrivileged(TAMSPrivilege.EDIT_SCHEDULE_LOCATION) == true || Numerics.GetBool(Session["CanUpdate"]) == true)
                        {
                            tempAction = "<img src='" + path + "images/edit.png' title='Edit' alt='Edit' onclick='return Edit(" + Numerics.GetDecimal(obj["schedule_location_id"]) + ");' /> &nbsp; ";
                        }
                        if (Numerics.GetBool(Session["CanDelete"]) == true || true)
                        {
                            //tempAction += "<img src='" + path + "images/delete.png' alt='Delete' title='Delete' onclick='Delete(" + Numerics.GetDecimal(obj["schedule_location_id"]) + ")' /> ";
                        }
                    }
            ...
            {
                new ExceptionLog().WriteLog("RequestPages/ScheduleLocationPage.aspx.GerAll()", 0, ex);
            }
            finally
            {
                objDAL.CloseConnection();
