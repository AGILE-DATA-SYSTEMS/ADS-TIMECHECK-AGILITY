...
{
    public partial class GeoTracking : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect(ResolveClientUrl("~/Dashboard/Geo-Tracking"));
            }

            DBAppSetting AppStng = new DBAppSetting();
            url = Request.Url.AbsoluteUri;
            if (url.ToLower().Contains("geotracking.aspx"))
                path = url.Substring(0, url.ToLower().IndexOf("dashboard/geotracking.aspx"));
            else
                path = url.Substring(0, url.ToLower().IndexOf("dashboard/geo-tracking"));
