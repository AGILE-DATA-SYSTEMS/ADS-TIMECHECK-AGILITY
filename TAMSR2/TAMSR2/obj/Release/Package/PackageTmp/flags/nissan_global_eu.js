//2013/11/20 nissan_global_eu ver1.40

var impAlc = location.href;
var impAdomain = document.domain;
var impAtitle = document.title;
var impAreferrer = document.referrer;

var impAlanguage = new Array();
impAlanguage = impAlc.split("/");
var impARSurl = impAlc.substring(0,13);

impAquery = impAlc.split("?");
var impAref = new Array();
var impAindexhtml=impAquery[0].replace("index.html","");
var impAhtm=impAindexhtml.replace("index.htm","");
var impAphp=impAhtm.replace("index.php","");

var SC_category_check1=impAlanguage[4]
var SC_category_check2=impAlanguage[4]+"/"+impAlanguage[5]
var SC_category_check3=impAlanguage[2]
...
/************************ ADDITIONAL FEATURES ************************
     Plugins
*/
if(impARSurl == "http://wwwpro"){
  var s_account="nissanjpglobaldev"
  var s=s_gi(s_account)
}else if(impARSurl == "http://wwwstg"){
  var s_account="nissanjpglobaldev"
  var s=s_gi(s_account)
}else if(impAlc.indexOf("nissan-wct.jazz.netgrid.jp") >-1){
  var s_account="nissanjpglobaldev"
  var s=s_gi(s_account)
}else if(impAdomain == "ntj.wa.xstep.biz"){
  var s_account="nissanjpglobaldev"
  var s=s_gi(s_account)
}else{
  var s_account="nissanjpglobal"
  var s=s_gi(s_account)
}
/************************** CONFIG SECTION **************************/
...
s.linkInternalFilters="javascript:,nissan-global.com,nml-public.net23.nml.jdc"
...
function s_doPlugins(s) {
  ...
  s.prop47="nissanjpglobal";
  /*LEAD*/
  s.prop49="90000000000000000000000000000000000000000000000000";
  s.prop38=s.getQueryParam('csid',':',window.parent.location.href);
  if(s.prop38.match(/\D+/)){
    s.prop38="";
  }
  if(s.prop49.match(/^\d{50}$/)){}else{
    s.prop49="";
  }
  if(s.prop37.match(/^\d{17}$/)){}else{
    s.prop37="";
  }
}
...
function sc_exe(scadid){
  /******************** redirect refferer(s_code) ********************/
  if(s.c_r("s_referrer") && s.c_r("s_referrer").match(/^http/)){
    s.referrer = s.c_r("s_referrer");
    s.c_w("s_referrer", "");
  }
  var targetref = s.referrer ? s.referrer : document.referrer;

  /******************** CODE TO PASTE(common) ********************/
  s.campaign=s.getQueryParam('scadid,sclisid,scnisid,scsocid,sclnkid',':');
  s.prop35=s.getQueryParam('rstid');

  /************* CODE TO PASTE **************/
  s.prop5="";
  s.pageName=impAtitle;
  s.eVar1=s.prop1="D=g";
  s.eVar2=impAquery[0];
  s.prop2="D=v2";
  s.prop3=impAdomain;
  ...
  if(typeof MF_search_phrase != 'undefined' && MF_search_phrase != ""){
    s.prop15=MF_search_phrase;
  }

  /************* referrer **************/
  /* CODE TO PASTE(getVisitStart_referrer) */
  var omnistart = s.getVisitStart("s_visit");
  s.prop26=s.referrer ? s.referrer : document.referrer;

  /* CODE TO PASTE(referrer) */
  if(targetref.indexOf("jdc") >-1 
  ||targetref.indexOf("nissan-global.com") >-1 
  ||targetref.indexOf("nml") >-1 
  ||targetref.indexOf("autech") >-1 
  ||targetref.indexOf("ris-asia") >-1){
  }else{
    s.prop50="D=c26";
    if(omnistart){
      s.prop6=s.prop7="D=c26";
    }
  }

  /******************** SESSION ID ********************/
  var sessionid=0;
  impAxx = Math.floor(Math.random() * 10000);
  ...
  impAdd = new Date();
  impAyy = impAdd.getYear();
  impAmm = impAdd.getMonth() + 1;
  impAaa = impAdd.getDate();
  impAhh = impAdd.getHours();
  impAnn = impAdd.getMinutes();
  impAss = impAdd.getSeconds();
  impAww = impAdd.getMilliseconds();

  if (impAyy < 2000) { impAyy += 1900; }
  if (impAmm < 10) { impAmm = "0" + impAmm; }
  if (impAaa < 10) { impAaa = "0" + impAaa; }
  if (impAhh < 10) { impAhh = "0" + impAhh; }
  if (impAnn < 10) { impAnn = "0" + impAnn; }
  if (impAss < 10) { impAss = "0" + impAss; }
  if(impAww < 100) { impAww = "0" + impAww; }
  if(impAww < 10) { impAww = "0" + impAww; }
  ...
  impAww+="";
  sessionid+="";
  sessionid = impAyy + impAmm + impAaa + impAhh + impAnn + impAss + impAxx;
  s.prop37=impAyy + impAmm + impAaa + impAhh + impAnn + impAss + impAww;

  /************* category **************/
  switch(SC_category_check1){
    case "BLUECITIZENSHIP":
      s.prop11="Blue Citizenship";
      break;
    case "ENVIRONMENT":
      s.prop11="Environment";
      break;
    case "CITIZENSHIP":
      s.prop11="Corporate Citizenshio";
      break;
    case "TECHNOLOGY":
      s.prop11="Technology";
      break;
    ...
    case "NEWS":
      ...
      break;
    case "IR":
      s.prop11="For Investor";
      break;
    case "PRODUCTS":
      s.prop11="Product Information";
      break;
    case "NISSAN":
      s.prop11="Product Information";
      break;
    case "INFINITI":
      ...
      break;
    case "NISSANCHANNEL":
      s.prop11="Nissan Channel";
      break;
    case "REPORTS":
      s.prop11="Reports";
      break;
    case "LICENSE":
      s.prop11="Licensing Business";
      break;
    case "NRC":
      s.prop11="Nissan Research Center";
      break;
    case "CSR":
      s.prop11="CSR(EN)";
      break;
  }
  switch(SC_category_check2){
    case "COMPANY/CSR":
      s.prop11="CSR(EN)";
      break;
  }
  switch(SC_category_check3){
    case "photos.nissan-global.com":
      s.prop11="Photo Gallery";
      break;
    case "blog.nissan-global.com":
      s.prop11="blog";
      break;
    case "reports.nissan-global.com":
      s.prop11="Reports";
      break;
  }
  ...
  if(omnistart){
    s.prop13=s.eVar13="D=c11";
    s.prop14=s.eVar14="D=c12";
    s.eVar21=sessionid;
  }

  var impAurlreplace=impAphp.replace("http://www.nissan-global.com/","");
  var impAurlreplace2=impAurlreplace.replace("https://www.nissan-global.com/","");
  impAurlreplace2=impAurlreplace2.replace(/http:\/\/|https:\/\/|www\.|www2\./g,"");
  if(impAlc.indexOf("blog.nissan-global.com/") > -1 || impAlc.indexOf("reports.nissan-global.com/") > -1 || impAlc.indexOf("photos.nissan-global.com/") > -1){
    if(impAquery[1]){
      var pagedecode=decodeURIComponent(impAquery[1]);
      impAurlreplace2=impAurlreplace2+"?"+pagedecode;
    }
  }
  var impAp10=0;
  var impAkkkkk=s.prop5;

  if(typeof impAkkkkk!='undefined'){
    impAp10="FL:" + s.prop5 + ":" + s.prop12 + ":" + s.prop11 + ":" + impAurlreplace2;
    s.pageName="FL:" + s.prop5 + ":" + impAtitle;
  }else{
    impAp10=s.prop12 + ":" + s.prop11 + ":" + impAurlreplace2;
  }

  var impAp11=impAp10.replace("undefined:","");
  var impAp12=impAp11.replace("undefined:","");

  s.eVar10=impAp12;
  s.prop10=s.eVar32=s.eVar33="D=v10";
  s.pageName = s.dropString(100, s.pageName);
  s.prop4=s.eVar4="D=pageName";

  var uniwc1=0;
  var timertt = setInterval(function(){uniset1()},100);
  function uniset1(){
    if(uniwc1==10){
      clearInterval(timertt);
      var s_code=s.t();if(s_code)document.write(s_code);
    }else{
      if(typeof ZID != 'undefined' && ZID != ""){
        s.prop36 = ZID;
        s.setCk("IDp6",ZID,"36500");
        clearInterval(timertt);
        var s_code=s.t();if(s_code)document.write(s_code);
      }else{
        uniwc1++;
...
function VL_Send(vlsend1,vlsend2,vlsend3,vlsend4){
  sc_exe(vlsend1);
}
