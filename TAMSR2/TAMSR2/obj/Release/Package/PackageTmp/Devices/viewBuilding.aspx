<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="ViewBuilding.aspx.cs" Inherits="TAMSR2.Devices.ViewBuilding" %>


<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    
    <title><%=Resources.Resource.building %> - <%=Resources.Resource.applicationName %></title>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
               ...
               <div class="span12">
                  ...
                  <div class="portlet box purple">
                     <div class="portlet-title">
                        <h4><i class="icon-reorder"></i>&nbsp; <%=Resources.Resource.building %></h4>
                     </div>
                     <div class="portlet-body form">
                        ...
                        <form action="#" id="form_sample_1" class="form-horizontal">
                           ...
                           <div class="control-group">
                              <label class="control-label"><%=Resources.Resource.longitude %></label>
                              <div class="controls">
                                 <input type="text" id="txtLongitude" name="longitude" data-required="1" class="span6 m-wrap"/>
                              </div>
                           </div>
                           <div class="control-group">
                              <label class="control-label"><%=Resources.Resource.latitude %></label>
                              <div class="controls">
                                 <input type="text" id="txtLatitude" name="latitude" data-required="1" class="span6 m-wrap"/>
                              </div>
                           </div>
                           <div class="control-group">
                              <label class="control-label"><%=Resources.Resource.radius %></label>
                              <div class="controls">
                                 <input type="text" id="txtRadius" name="radius" data-required="1" class="span6 m-wrap"/>
                              </div>
                           </div>
                           <div class="control-group">
                              <label class="control-label"><%=Resources.Resource.choose %> <%=Resources.Resource.zones %><span class="required">*</span></label>
                              <div class="controls">
                                   <select id="ddZonesList" style="width: 220px;" name="ZonesList" class="span6 chosen" data-with-diselect="1" data-placeholder="<%=Resources.Resource.choose %> <%=Resources.Resource.zones %>" tabindex="1">
                                   </select>
                               </div>
                           </div>
                           <div class="control-group">
                               ...
                               <div class="controls">
                                   <input id="chkboxIsActive" type="checkbox" name="isActive" /> <%=Resources.Resource.active %>
                               </div>
                           </div>
         ...
         <div class="container-fluid">
			...
			<ul class="breadcrumb">
				...
				<li><a href="#"><%=Resources.Resource.building %></a></li>					
            ...
            <div class="row-fluid">
               <div class="span12">
                  ...
                  <div class="portlet box light-grey">
                     <div class="portlet-title">
                        <div class="caption"><i class="icon-globe"></i><%=Resources.Resource.building %></div>
                        <div class="tools">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="sample_1">
                           <thead>
                              <tr style="background-color: #e8e3e3;">
                                 ...
                                 <th style="text-align: center;"><%=Resources.Resource.longitude %></th>
                                 <th style="text-align: center;"><%=Resources.Resource.latitude %></th>
                                 <th style="text-align: center;"><%=Resources.Resource.zones %></th>
                                 <th style="text-align: center;"><%=Resources.Resource.active %></th>
                                 <th style="text-align: center;"><%=Resources.Resource.deleted %></th>
  ...
  <script src='../Assets/<%= languageStr %>/plugins/jquery-validation/dist/jquery.validate.min.js' type='text/javascript'></script>
   ...
   <script type="text/javascript">
       ...
       $(document).ready(function () {
           ...
           $("#li_DevicesBuilding").addClass("active");
           ...
           var oTable = $('#sample_1').dataTable({
               ...
               "aaSorting": [[1, "desc"]],
               "sAjaxSource": '<%= Page.ResolveClientUrl("../RequestPages/BuildingsPage.aspx?key=getAll") %>'
           });
       ...
       function LoadZones() {
           ...
           $.ajax({
               type: 'POST',
               data: d,
               url: '<%= Page.ResolveClientUrl("~/RequestPages/ZonesPage.aspx") %>',
               success: function (html) {
                   $("#ddZonesList").html(html);
                   $("#ddZonesList").trigger("liszt:updated");
              ...
              url: '<%= Page.ResolveClientUrl("../RequestPages/BuildingsPage.aspx") %>',
               success: function (html) {
                   if (html != "") {
                       ...
                       $("#txtCode").val(j.Code);
                       $("#txtLongitude").val(j.Longitude);
                       $("#txtLatitude").val(j.Latitude);
                       $("#txtRadius").val(j.Radius);
                       if (j.is_active + "" == "1" || j.is_active + "" == "True" || j.is_active + "" == "true" || j.is_active + "" == "TRUE") {
                           $("#chkboxIsActive").attr("checked", "checked");
                           $("#uniform-chkboxIsActive span").addClass("checked");
                       }
                       if (j.is_active + "" == "0" || j.is_active + "" == "False" || j.is_active + "" == "false" || j.is_active + "" == "FALSE") {
                           $("#chkboxIsActive").attr("unchecked", "unchecked");
                           $("#uniform-chkboxIsActive span").removeClass("checked");
                       }
                       $('#ddZonesList option[value=' + j.zone_id + ']').attr('selected', 'selected');
                       $("#ddZonesList").trigger("liszt:updated");
                       
                       /* if (j.is_deleted + "" == "1" || j.is_deleted + "" == "true" || j.is_deleted + "" == "TRUE") {
                            $("#chkboxIsActive").attr("checked", "checked");
                            $("#uniform-chkboxIsActive span").addClass("checked");
                        }*/
                   }
       ...
       function Save() {
           ...
           var building_zone_id = $("#ddZonesList").val();
           var code = $("#txtCode").val();
           var is_active = $("#chkboxIsActive").is(":checked");
           if ($('#form_sample_1').validate().form() == false) {
               return false;
           }
           var v = "key=save" + "&id=" + id + "&code=" + code + "&is_active=" + is_active;
           v += "&descriptionEn=" + descriptionEn + "&descriptionAr=" + descriptionAr + "&building_zone_id=" + building_zone_id + "&txtLongitude=" + longitude + "&txtLatitude=" + latitude+ "&txtRadius=" + radius;
